...
class BlockChain {
public:
    BlockChain(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, int genesis  = 1 );
    Block getBlock(int index);
    // getBlock(string hash); //not implemented
    int getNumOfBlocks(void);
    int addBlock(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, int index, string prevHash, string hash, string nonce, vector<string> &merkle);
    string getLatestBlockHash(void);
    void toString(void);
    string toJSON(void);
    int replaceChain(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, json chain);
private:
...
BlockChain::BlockChain(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, int genesis){
    if (genesis == 0) {
        vector<string> v;
        ...
        this -> blockchain.push_back(std::make_unique<Block>(0,string("00000000000000"),hash_nonce_pair.first,hash_nonce_pair.second,v, sm4Wrapper, sm2Wrapper));
...
int BlockChain::addBlock(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, int index, string prevHash, string hash, string nonce, vector<string> &merkle) {
    string header = to_string(index) + prevHash + getMerkleRoot(merkle) + nonce;
    if ( (!sha256(header).compare(hash)) && (index == blockchain.size())) {
        printf("Block hashes match --- Adding Block %s \n",hash.c_str());
        this->blockchain.push_back(std::make_unique<Block>(index,prevHash,hash,nonce,merkle, sm4Wrapper, sm2Wrapper));
        return 1;
    }
...
int BlockChain::replaceChain(SMWrapper::SM4Wrapper& sm4Wrapper, SMWrapper::SM2Wrapper& sm2Wrapper, json chain) {
    ...
    for (int a = 1; a <chain["length"].get<int>(); a++ ){
        ...
        this->addBlock(sm4Wrapper, sm2Wrapper, block["index"],block["previousHash"],block["hash"],block["nonce"],data);
    } 
    return 1;
}
